#!/usr/bin/env python3
"""
Gateway watchdog daemon for Friday Router.
...
def _guard_script():
    return Path(__file__).resolve().parent / "gateway_guard.py"
...
def run_recovery() -> None:
    ...
    # 1. status --json
    try:
        _run([sys.executable, guard, "status", "--json"], timeout=12, capture=True)
